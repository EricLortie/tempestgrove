...
<div class="container-fluid">
	<div class="row">
        ...
        { if( have_rows('races', get_id_by_slug('codex-races')) ): }
          { $races = array(); }
          { while( have_rows('races',get_id_by_slug('codex-races')) ): the_row(); }
            ...
            <script type="text/javascript">
              builder_data.races[`{ echo $name; }`] = {
                name: `{ echo $name }`,
                lifespan: `{ echo $life_span }`,
                racial_characteristics: `{ echo $racial_characteristics }`,
                description: `{ echo $description }`,
                advantages: `{ echo $advantages }`,
                disadvantages: `{ echo $disadvantages }`,
                frag_cost: `{  echo $frag_cost; }`
              }
            </script>

          { endwhile; }
          { //usort($races, 'sortByOption'); }
        { endif; }

        { if( have_rows('classes', get_id_by_slug('codex-classes')) ): }
          { $classes = array(); }
          { $vocations = array(); }
          { $occupations = array(); }
          { while( have_rows('classes', get_id_by_slug('codex-classes')) ): the_row(); }
            ...
            if ( $optional && in_array('vocation', $optional) ) {
              ...
              <script type="text/javascript">
                builder_data.vocations[`{ echo $name; }`] = {
                  name: `{ echo $name }`,
                  description: `{ echo $description }`
                }
              </script>
              ...
              <script type="text/javascript">
                builder_data.occupations[`{ echo $name; }`] = {
                  name: `{ echo $name }`,
                  description: `{ echo $description }`
                }
              </script>
              ...
              <script type="text/javascript">
                builder_data.pc_classes[`{ echo $name; }`] = {
                  name: `{ echo $name }`,
                  description: `{ echo $description }`
                }
              </script>
        ...
        { if( have_rows('spells', get_id_by_slug('codex-spells')) ): }
          ...
          { while( have_rows('spells', get_id_by_slug('codex-spells')) ): the_row(); }
            ...
            { $name = preg_replace('<type>', '[type]', get_sub_field('name')); }
            { $sphere = get_sub_field('sphere'); }
            { $incant = preg_replace('<type>', '[type]', get_sub_field('incant')); }
            { $level = get_sub_field('level'); }
            { $duration = get_sub_field('duration'); }
            { $desc = preg_replace('<type>', '[type]', get_sub_field('description')); }

            <script type="text/javascript">
              if(typeof builder_data.spells[`{ echo $sphere; }`] == "undefined"){
                builder_data.spells[`{ echo $sphere; }`] = {}
              }
              if(typeof builder_data.spells[`{ echo $sphere; }`][`{ echo $level; }`] == "undefined"){
                builder_data.spells[`{ echo $sphere; }`][`{ echo $level; }`] = []
              }
              builder_data.spells[`{ echo $sphere; }`][`{ echo $level; }`].push({
                name: `{ echo $name }`,
                level: `{ echo $level }`,
                sphere: `{ echo $sphere }`,
                incant: `{ echo $incant }`,
        ...
        { if( have_rows('spell_spheres', get_id_by_slug('codex-magic')) ): }
          { $count = 0; }
          { while( have_rows('spell_spheres', get_id_by_slug('codex-magic')) ): the_row(); }

            { build_skill_row($post, $count, "spell_sphere"); }
        ...
        { if( have_rows('skills', get_id_by_slug('codex-racial-abilities')) ): }
          { $s_count = 0; }
          { while( have_rows('skills', get_id_by_slug('codex-racial-abilities')) ): the_row(); }

            { build_skill_row($post, $s_count, "race_skill"); }

          { endwhile; }
        { endif; }

        { if( have_rows('skills', get_id_by_slug('codex-class-skills')) ): }
          { $s_count = 0; }
          { while( have_rows('skills', get_id_by_slug('codex-class-skills')) ): the_row(); }

            { build_skill_row($post, $s_count, "class_skill"); }
        ...
        { if( have_rows('skills', get_id_by_slug('codex-skills')) ): }
          { $s_count = 0; }
          { while( have_rows('skills', get_id_by_slug('codex-skills')) ): the_row(); }

            { build_skill_row($post, $s_count, "skill"); }


          { endwhile; }
        { endif; }

        { if( have_rows('skills', get_id_by_slug('codex-frag-skills')) ): }
          { $s_count = 0; }
          { while( have_rows('skills', get_id_by_slug('codex-frag-skills')) ): the_row(); }

            { build_skill_row($post, $s_count, "frag skill"); }
        ...
        <div id="launcher" class="row">
          <div class="col-sm-12">
            ...
            <div id="menu_launcher" class="row bg_{ echo rand(1,5); }">
              <div class="col-sm-12">
                <div id="menu_items">
                  <div id="menu_primary" class="row">
                    ...
                    <div class="blog-post text-center" style="">
                      <div id="category-menu" class="col-sm-12 { echo (wp_is_mobile()) ? "mobile" : "desktop"; }">
                        ...
                        <p style="margin-bottom:3rem;"><span class="full-width styled-dropdown custom-dropdown custom-dropdown--red">
                          <select id="category-dropdown" class="full-width builder_selector custom-dropdown__select custom-dropdown__select--red">
                            <option>---</option>
                            { $categories = array("Crafter", "Defender", "Attacker", "Damage Dealer", "Tank", "Guard", "Healer", "Passifist", "Hero", "Villain", "Storyteller", "Politician", "Merchant", "Jack of All Trades", "Specialist", "Observer", "Other"); }
                    ...
                    <div class="blog-post text-center col-sm-12 login_element" style="margin-bottom:3rem;">
                      ...
                      <p class="fb_login_placeholder" style="padding-top: 9px;"><img src="{ echo get_template_directory_uri(); }/inc/loading_spinner.gif" style="height:20px;"/> Loading</p>
            ...
            <div id="selector_elements" class="col-sm-12">
              <div id="race_selector" class="row">
                ...
                <div id="race-menu" class="col-sm-12 { echo (wp_is_mobile()) ? "mobile" : "desktop"; }">
                  <p><span class="custom-dropdown custom-dropdown--red">
                    ...
                    <option>Select a race</option>
                      ...
                      foreach ($races as $race) { }
                        ...
                        <option value="{ echo $race->name; }">{ echo $race_string }</option>
                      {  }}
                    </select>
                  </span></p>
                </div>
                ...
                <div id="race-content" class="col-sm-12">
                  {
                  foreach ($races as $race) { }
                    <div class="race content repeater-content" data-name="{ echo $race->name; }">
                      <h2>{ echo $race->name; }</h2>

                      <div class="row repeater-row">
                        <div class="col-sm-4 column race_meta">

                          <img src="{ echo $race->photo }" class="img-responsive builder_photo" alt="{ echo $race->name }" title="{ echo $race->name }"/>

                          <div class="blog-post text-center" style="margin-bottom:3rem;">
                            <a id="select_race" href="#" title="Select Race" data-race="{ echo $race->name; }" data-frag_cost="{ echo $race->frag_cost; }" class="builder_selector blog-post-button locked">
                              Select { echo $race->race_string; }
                            </a>
                          </div>

                          <h4>Lifespan</h4>

                          <p>{ echo $race->life_span; }</p>

                          <h4>Racial Characteristics</h4>

                          <p>{ echo $race->racial_characteristics; }</p>
                        </div>
                        <div class="col-sm-8 column race_info { echo (wp_is_mobile()) ? "mobile" : "desktop"; }">

                          <h4>Descriptipn</h4>
                          { echo $race->description; }

                          <h4>Advantages</h4>
                          { echo $race->advantages; }

                          <h4>Disadvantages:</h4>
                          { echo $race->disadvantages; }
              ...
              <div id="class_selector" class="row">
                ...
                <div id="class-menu" class="col-sm-12 { echo (wp_is_mobile()) ? "mobile" : "desktop"; }">
                  <p><span class="custom-dropdown custom-dropdown--red">
                    <select id="class-dropdown" class="builder_selector custom-dropdown__select custom-dropdown__select--red">
                    <option>Select a class</option>
                      {
                      foreach ($classes as $class) { }
                        { $class_string = ($class->frag_cost > 0) ? $class->name . " (" . $class->frag_cost . " FRAGS)" : $class->name; }
                        <option value="{ echo $class->name; }">{ echo $class_string }</option>
                      { } }
                    </select>
                  </span></p>
                </div>
                ...
                <div id="class-content" class="col-sm-12">
                  { foreach ($classes as $pc_class) { }
                    <div class="class content repeater-content" data-name="{ echo $pc_class->name; }">
                      <h2>{ echo $pc_class->name; }</h2>

                      <div class="row repeater-row">
                        <div class="col-sm-4 column class_meta">

                          <img src="{ echo $pc_class->photo }" class="img-responsive builder_photo" alt="{ echo $pc_class->name }" title="{ echo $pc_class->name }"/>

                        </div>
                        <div class="col-sm-8 column class_info">

                          <h4>Descriptipn</h4>
                          { echo $pc_class->description; }


                          <div class="blog-post text-center" style="margin-bottom:3rem;">
                            <a id="select_class" href="#" title="Select Class" data-class="{ echo $pc_class->name; }" data-frag_cost="{ echo $pc_class->frag_cost; }" data-cost-ele="{ echo strtolower(substr($pc_class->name, 0, 3)); }_cost" class="builder_selector blog-post-button locked">Select { echo $pc_class->name; }</a>
                          </div>
                ...
                <div class="cb_data">
                  ...
                  <div id="cb_armour_details" class="cb_display_content"  data-tab="Armour">
                    ...
                    </ul>
                    {
                      ...
                      <div id="armour_{ echo $slot; }" val1="0" val2="0" class="armour_slot">
                        <div class="row">
                          <div class="col-sm-2">
                            { echo $slot; }
                          </div>
                          ...
                          <div class="col-sm-3">
                            <span id="ap_val_{ echo $slot; }">0</span> pts
                          </div>
                          <div class="col-xs-2">
                            <i class="fa fa-toggle-down expand_armour" armour="{ echo $slot; }" aria-hidden="true"></i>
                            <i class="fa fa-toggle-up close_armour hidden" armour="{ echo $slot; }" aria-hidden="true"></i>
                          </div>
                        </div>
                        <div class="row armour_slot_data armour_slot_data_1">
                          <div class="col-xs-2">
                            <span id="armour_{ echo $slot; }_val_1">0</span> pts
                          </div>
                          <div class="col-xs-8">
                            <span id="armour_{ echo $slot; }_desc_1">No Armour</span>
                          </div>
                          <div class="col-xs-2">
                            <i class="fa fa-plus-square armour_toggle show_armour_modal" slot="{ echo $slot; }" level="1" aria-hidden="true"></i>
                          </div>
                        </div>
                        <div class="row armour_slot_data armour_slot_data_2 locked">
                          <div class="col-xs-2">
                            <span id="armour_{ echo $slot; }_val_2">0</span> pts
                          </div>
                          <div class="col-xs-8">
                            <span id="armour_{ echo $slot; }_desc_2">No Armour</span>
                          </div>
                          <div class="col-xs-2 action_buttons">
                            <i class="fa fa-plus-square armour_toggle show_armour_modal" slot="{ echo $slot; }" level="2" aria-hidden="true"></i>
                          </div>
                        </div>
                      </div>
                    { }}
                   ...
                   { foreach ($legend_items as $li){ }
                      <div class="col-sm-4">
                        <div class="col-xs-2 text-center">
                          <i class="fa fa-{ echo $li[0]}" aria-hidden="true"></i>
                        </div>
        ...
        <script type="text/javascript" src="{ site_url(); }/wp-content/themes/illdy/layout/js/character-builder.js"></script>
...
<div id="blog">
    ...
    <div id="occupation_selector" class="row">
      ...
      <div id="occupation-menu" class="col-sm-12 { echo (wp_is_mobile()) ? "mobile" : "desktop"; }">
        <p><span class="custom-dropdown custom-dropdown--red">
      ...
      <div id="occupation-content" class="col-sm-12">
        { foreach ($occupations as $pc_class) { }
          <div class="occupation content repeater-content" data-name="{ echo $pc_class->name; }">
            <h2>{ echo $pc_class->name; }</h2>
            ...
            <div class="row repeater-row">
              ...
              <div class="col-sm-8 column class_info">
                ...
                <div class="blog-post text-center" style="margin-bottom:3rem;">
                  <a id="select_occupation" href="#" title="Select Class" data-class="{ echo $pc_class->name; }" data-frag_cost="{ echo $pc_class->frag_cost; }" data-cost-ele="{ echo strtolower(substr($pc_class->name, 0, 3)); }_cost" class="builder_selector occupation blog-post-button locked">Select { echo $pc_class->name; }</a>
                </div>
    ...
    <div id="vocation_selector" class="row">
      ...
      <div id="vocation-menu" class="col-sm-12 { echo (wp_is_mobile()) ? "mobile" : "desktop"; }">
        <p><span class="custom-dropdown custom-dropdown--red">
      ...
      <div id="occupation-content" class="col-sm-12">
        { foreach ($vocations as $pc_class) { }
          <div class="vocation content repeater-content" data-name="{ echo $pc_class->name; }">
            <h2>{ echo $pc_class->name; }</h2>
            ...
            <div class="row repeater-row">
              ...
              <div class="col-sm-8 column class_info">
                ...
                <div class="blog-post text-center" style="margin-bottom:3rem;">
                  <a id="select_vocation" href="#" title="Select Class" data-class="{ echo $pc_class->name; }" data-frag_cost="0" data-cost-ele="{ echo strtolower(substr($pc_class->name, 0, 3)); }_cost" class="builder_selector vocation blog-post-button locked">Select { echo $pc_class->name; }</a>
                </div>
    ...
    <div class="modal-content">
        ...
        <div class="tab-content">
          { for ($day=1;$day<=3;$day++){ }
            ...
            <div role="tabpanel" id="{ echo $label;}" class="{ if($day == 1) { echo 'active'; } } tab-pane fade in">
              <div class="row spell_circle disabled elf_only" circle="elf">
                <div class="col-sm-1">
                  <label class="elf-circle">High Elf</label>
                </div>
                <div class="col-sm-2">
                  <div class="spell_slot" circle="elf" slot="{ echo $slot; }" day="{ echo $day; }">
                    <p class="spell_name" circle="elf" slot="{ echo $slot; }" day="{ echo $day; }">No spell selected.</p>
                    <p class="change_spell">Change</p>
                  </div>
                </div>
              </div>

              { for ($circle=9;$circle>=1;$circle--){ }
                <div class="row spell_circle disabled" circle="{ echo $circle; }">
                  <div class="col-sm-1">
                    <label circle="{ echo $circle;}">{ echo $circle;}</label>
                  </div>
                  { for ($slot=1;$slot<=5;$slot++){ }
                    <div class="col-sm-2">
                      <div class="spell_slot disabled" circle="{ echo $circle; }" slot="{ echo $slot; }" day="{ echo $day; }">
                        <p class="spell_name" circle="{ echo $circle; }" slot="{ echo $slot; }" day="{ echo $day; }">No spell selected.</p>
                        <p class="change_spell">Change</p>
                      </div>
                    </div>
                  { } }
        ...
        <div class="row">
          <div class="col-sm-12">
            ...
            <select id="armour_type">
              { foreach([[0, "No Armour"], [1, 'Leather'],[2, 'Boiled/Studded Leather'],[3, 'Chain/Scale'],[4, 'Plate']] as $a) { }
                <option value="{ echo $a[0];}" type="{ echo $a[1];}">{ echo $a[1];}</option>
              { } }
            </select>
          </div>
        </div>
